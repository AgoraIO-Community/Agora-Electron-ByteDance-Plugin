...
int ByteDancePlugin::setParameter(const char *param)
{
	...
	if (d.HasMember("plugin.bytedance.licenseDir")) {
		Value& licenseDir = d["plugin.bytedance.licenseDir"];
		if (!licenseDir.IsString()) {
			return -101;
		}
		mLicenseDirPath = std::string(licenseDir.GetString());
	}

	if (d.HasMember("plugin.bytedance.licenseData")) {
		Value& licenseData = d["plugin.bytedance.licenseData"];
		if (!licenseData.IsString()) {
			return -101;
		}
		mLicenseData = std::string(licenseData.GetString());
	}

	if (d.HasMember("plugin.bytedance.licenseDigest")) {
		Value& digest = d["plugin.bytedance.licenseDigest"];
		if (!digest.IsString()) {
			return -101;
		}
		mLicenseDigest = std::string(digest.GetString());
	}
	...
	if (d.HasMember("plugin.bytedance.licensePath")) {
		...
		mLicensePath = std::string(licensePath.GetString());
	}
...
const char* ByteDancePlugin::getParameter(const char* key)
{
	...
	if (strncmp(key, "plugin.bytedance.authdata", strlen(key)) == 0) {
	...
	else if (strncmp(key, "plugin.bytedance.writeLicense", strlen(key)) == 0) {
		auto re_hash = HMAC_SHA256(
			Misc::concat(mLicenseSecret, ""),
			Misc::concat(mLicenseData, "")
		);
		...
		if (hexitize(re_hash).compare(mLicenseDigest) == 0) {
			...
			if (sep != last_str) {
				lic_file_path += sep;
				lic_file_path += lic_file_name;
			}
			else {
				lic_file_path += lic_file_name;
			}

			bool dir_is_exist = false;
			#if defined(_WIN32)
				if (_access(mLicenseDirPath.c_str(), 0) != -1) {
					dir_is_exist = true;
				}
			#else
				// *NIX
				if (access(mLicenseDirPath.c_str(), 0) != -1) {
					dir_is_exist = true;
				}
			#endif
			...
			if (!dir_is_exist) {
				writer.Int(-101);
				writer.Key("msg");
				std::string str1 = "no such directory ";
				str1.append(mLicenseDirPath);
				writer.String(str1.c_str());
				writer.Key("path");
				writer.String("");
			}
			else {
				...
				writer.Key("path");
				writer.String(lic_file_path.c_str());
			}
		}
		else {
			writer.Int(-102);
			writer.Key("msg");
			writer.String("check license failed, please refetch license data");
			writer.Key("path");
			writer.String("");
		}
		writer.EndObject();
